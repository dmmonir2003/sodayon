...
export default function AgeFinderPage() {
  ...
  const currentRangeId = searchParams.get("range") || "1-3";
  const activeAge = AGES.find(a => a.id === currentRangeId) || AGES[1];

  const handleAgeChange = (id: string) => {
    router.push(`/shop/age?range=${id}`);
  };
    ...
    <div className="bg-slate-50 dark:bg-slate-900 min-h-screen pb-24">
      ...
      <div className="bg-primary-600 text-white py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-3xl md:text-5xl font-bold font-heading mb-4">Find The Perfect Toy By Age</h1>
          <p className="text-primary-100 max-w-2xl mx-auto">
            Children's brains develop rapidly. Filtering toys by the precise age stage ensures max safety and fun. Select an age below to filter the catalog.
          </p>
        </div>
      </div>
          ...
          {AGES.map((age) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-xs font-medium uppercase tracking-wider">{age.label}</span>
              </button>
            )
          })}
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-slate-800 dark:text-white">Toys for {activeAge.range}</h2>
          <SortDropdown />
        </div>
          ...
          <div className="flex-1">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {[1,2,3,4,5,6].map((prod) => (
                <ProductCard 
                  key={prod} 
                  name={`${activeAge.label} Educational Toy ${prod}`} 
                  price={`$${(19.99 + prod * 5).toFixed(2)}`} 
                  img={`bg-primary-${prod}00`} 
                />
              ))}
            </div>
          </div>
